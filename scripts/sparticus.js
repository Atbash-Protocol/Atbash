...
async function main() {
    ...
    // Deploy OHM
    const OHM = await ethers.getContractFactory('BASHERC20Token');
    const ohm = await OHM.deploy();

    // Deploy DAI
    const DAI = await ethers.getContractFactory('MockERC20');
    const dai = await DAI.deploy("dai", "dai", "10000000000000000000000000000000");

    // Deploy Frax
    const Frax = await ethers.getContractFactory('MockERC20');
    const frax = await Frax.deploy('frax', 'frax', '10000000000000000000000000000000' );

    // Deploy 10,000,000 mock DAI and mock Frax
    await dai.mint( initialMint );
    await frax.mint( initialMint );
    ...
    // Deploy bonding calc
    const OlympusBondingCalculator = await ethers.getContractFactory('ATBASHBondingCalculator');
    ...
    // Deploy sOHM
    const SOHM = await ethers.getContractFactory('sBASH');
