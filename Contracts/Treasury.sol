...
contract bashTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, Sbash }
    ...
    uint public sbashQueue; // Delays change to sbash address
    ...
    constructor (
       ...
       isLiquidityToken[ _bashDAI ] = true;
       liquidityTokens.push( _bashDAI );
    ...
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IbashERC20( bash ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sbash ).balanceOf( msg.sender ); // Can only borrow against sbash held
    ...
    /**
        ...
        @param _amount uint
     */
    function repayDebtWithbash( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IbashERC20( bash ).burnFrom( msg.sender, _amount );
    ...
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            ...
            value_ = _amount.mul( 10 ** IERC20( bash ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.sbash ) { // 9
            sbashQueue = block.number.add( blocksNeededForQueue );
        } else return false;
