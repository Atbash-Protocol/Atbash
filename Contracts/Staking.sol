...
interface Isbash {
    function rebase( uint256 bashProfit_, uint epoch_) external returns (uint256);
...
contract bashStaking is Ownable {
    ...
    function stake( uint _amount, address _recipient ) external returns ( bool ) {
        ...
        IERC20( bash ).safeTransferFrom( msg.sender, address(this), _amount );
    ...
    function claim ( address _recipient ) public {
        ...
        if ( epoch.number >= info.expiry && info.expiry != 0 ) {
            ...
            IWarmup( warmupContract ).retrieve( _recipient, Isbash( sbash ).balanceForGons( info.gons ) );
        }
    }
    ...
    function forfeit() external {
        ...
        IWarmup( warmupContract ).retrieve( address(this), Isbash( sbash ).balanceForGons( info.gons ) );
    ...
    function rebase() public {
        if( epoch.endBlock <= block.number ) {
            ...
            uint staked = Isbash( sbash ).circulatingSupply();
